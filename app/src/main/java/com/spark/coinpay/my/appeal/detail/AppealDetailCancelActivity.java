...
import static com.spark.coinpay.my.appeal.AppealCancelFragment.appealApplyAchiveVo;
...
public class AppealDetailCancelActivity extends BaseActivity {
    ...
    protected void initData() {
        super.initData();
        if (appealApplyAchiveVo != null) {
            payDatas = new Gson().fromJson(appealApplyAchiveVo.getPayData(), new TypeToken<List<PayData>>() {
            }.getType());
    ...
    private void initContent() {
        int direction = appealApplyAchiveVo.getDirection();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

        if (appealApplyAchiveVo.getStatus() != null) {//状态0-未处理 1-已处理
            if (appealApplyAchiveVo.getStatus() == 0) {
                tvDo.setText("未处理");
                tvDoTime.setText("");
            } else {
                tvDo.setText("已处理");
                tvDoTime.setText(simpleDateFormat.format(appealApplyAchiveVo.getDealWithTime()));
            }
        } else {
            tvDo.setText("");
            tvDoTime.setText("");
        }
        tvAppealNum.setText(appealApplyAchiveVo.getId() + "");
        tvCreateTime.setText(simpleDateFormat.format(appealApplyAchiveVo.getCreateTime()));
        tvContent.setText(appealApplyAchiveVo.getContent());
        if (appealApplyAchiveVo.getIsSuccess() != null) {//申诉结果0 败诉 1胜诉 2已关闭
            if (appealApplyAchiveVo.getIsSuccess() == 0) {
                tvResult.setText(MyApplication.getAppContext().getString(R.string.str_appeal_success));
            } else if (appealApplyAchiveVo.getIsSuccess() == 1) {
                tvResult.setText(MyApplication.getAppContext().getString(R.string.str_appeal_fail));
            } else if (appealApplyAchiveVo.getIsSuccess() == 2) {
                tvResult.setText(MyApplication.getAppContext().getString(R.string.str_appeal_close));
            }
        } else {
            tvResult.setText(MyApplication.getAppContext().getString(R.string.str_appeal_cancel));
        }

        tvTypeValue.setText(MathUtils.subZeroAndDot(appealApplyAchiveVo.getAmount() + ""));
        tvAmount.setText(MathUtils.subZeroAndDot(appealApplyAchiveVo.getNumber() + ""));
        tvRate.setText(MathUtils.subZeroAndDot(appealApplyAchiveVo.getPrice() + ""));
        tvOrderCreateTime.setText(simpleDateFormat.format(appealApplyAchiveVo.getOrderCreateTime()));
        if (appealApplyAchiveVo.getReleaseTime() != null) {
            tvOrderConfirmTime.setText(simpleDateFormat.format(appealApplyAchiveVo.getReleaseTime()));
        } else {
            tvOrderConfirmTime.setText("");
        }

        tvPayRefer.setText(appealApplyAchiveVo.getPayRefer());
        tvOrderNum.setText(appealApplyAchiveVo.getOrderId());
        ...
        if (direction == 1) {//卖币
            ...
            tvTypeTag.setText(String.format(getString(R.string.str_pay_usdt), appealApplyAchiveVo.getCoinId()));
            ...
            tvTypeTag.setText(String.format(getString(R.string.str_get_usdt), appealApplyAchiveVo.getCoinId()));
